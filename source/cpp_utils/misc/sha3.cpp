...
/// same as reset()
_SHA3_IMPL::_SHA3_IMPL(Bits bits)
...
void _SHA3_IMPL::processBlock(const void* data)
{
...
void _SHA3_IMPL::add(const void* data, size_t numBytes)
{
...
std::string _SHA3_IMPL::getHash()
{
  ...
  // _SHA3_IMPL-224's last entry in m_hash provides only 32 bits instead of 64 bits
...
std::string _SHA3_IMPL::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
std::string _SHA3_IMPL::operator()(const std::string& text)
{
  reset();
